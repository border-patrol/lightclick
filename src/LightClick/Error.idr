||| Stuff that goes wrong, and how to make it pretty.
|||
||| Module    : Error.idr
...
PrettyError Direction.Valid.Error where
  toString InputCannotSend
    = "Input cannot send"
  toString DirIsSame
    = "Directions are the same"
  toString OutputCannotReceive
    = "Output ports cannot receive"

export
PrettyError Direction.Safe.Error where
  toString (VCError x)
    = toString x
  toString PossibleFeedbackFromReceiver
    = "Possible Feedback from Receiver."
  toString PossibleFeedbackFromSender
    = "Possible Feedback from Sender."
...
namespace Compatibility
  ...
  namespace Data
    ...
    toString Mismatch
      = "The data types are wildly different."
    ...
    toString (MismatchStructureFieldType pos err)
      = "The union/struct differs at position "
        <+> show pos
        <+> ". With Error:\n"
        <+> toString err

    toString (MismatchStructureFieldLabel pos x y)
      = "The labels differ at position "
        <+> show pos
        <+> ". With Error:\n"
        <+> show x
        <+> " & "
  ...
  namespace Port
    ...
    PrettyError Port.Error where
      ...
      toString (InCompatSensitivity err)
        = toString err
      toString (InCompatDirection   err)
        = toString err
      toString (InCompatWTypes      err)
        = toString err
      toString (InCompatDTypes      err)
        = toString err
...
namespace LightClick
  ...
  PrettyError LightClick.Error where
    ...
    toString (Err loc msg)
      = unlines [show loc,msg]
    toString (NotSupposedToHappen Nothing)
      = "Not Supposed to Happen."

    toString (NotSupposedToHappen (Just s))
      = "Not Supposed to Happen. " <+> s
    ...
    toString (UnusedPorts Nil)
      = "Not supposed to happen: Unused port error is empty."
