...
convertDecls (MkDecl binder expr :: rest) with (convertDecls rest)
  convertDecls (MkDecl binder expr :: rest) | Left l = Left $ Nested "Converting Decls" l
  convertDecls (MkDecl binder expr :: rest) | Right (ctxt ** MkPair ps ds) with (convertDecl ds binder expr)
    convertDecls (MkDecl binder expr :: rest) | Right (ctxt ** MkPair ps ds) | (Left l) = Left l
...
convertExpr env (Global label ty) with (env)
  convertExpr env (Global label ty) | (MkEnv global decls local) with (isElem (label,ty) global)
    convertExpr env (Global label ty) | (MkEnv global decls local) | (Yes idx) = pure (Global label idx)
    convertExpr env (Global label ty) | (MkEnv global decls local) | (No contra) = Left $ General $ unwords ["Attempting to generate Global ref failed:", show label]
...
convertExpr env (Let this beThis {typeE} withType {ty} inThis) with (validLet typeE ty)
  ...
  convertExpr env (Let this beThis {typeE} withType {ty} inThis) | No x = Left $ General $ unwords ["Invalid Let:", show typeE, show ty]
...
convertExpr env (Seq x y) with (convertExpr env x)
  ...
  convertExpr env (Seq x y) | Right x' with (convertExpr env y)
    convertExpr env (Seq x y) | Right x' | Left err = Left err
...
convertSpec (MkMSVIRSpec decls expr) with (convertDecls decls)
  convertSpec (MkMSVIRSpec decls expr) | (Left l) = Left $ Nested "Attempting to build decls" l
  convertSpec (MkMSVIRSpec decls expr) | (Right (gs ** (prfVDecls,decls'))) with (convertExpr (MkEnv gs decls' Nil) expr)
    convertSpec (MkMSVIRSpec decls expr) | (Right (gs ** (prfVDecls,decls'))) | (Left l) = Left $ Nested "Attempted to build Spec" l
    convertSpec (MkMSVIRSpec decls expr) | (Right (gs ** (prfVDecls,decls'))) | (Right r) = pure (MkSpec decls' r)
