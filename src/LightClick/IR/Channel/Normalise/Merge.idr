||| Find all the module instantiations and merge them to a single
||| declaration.
|||
||| Module    : Merge.idr
...
updateState RepeatedName y
  = RepeatedName

updateState (UniqueNames xs) y with (isElem y xs)
  updateState (UniqueNames xs) y | (Yes prf)
    = RepeatedName
  updateState (UniqueNames xs) y | (No contra)
    = UniqueNames (y::xs)

updateState InlineModule y
  = InlineModule

||| A ChannelIR specification is in module normal form if each
||| CModuleInst has a unique type.
|||
||| Returns nothing if a name is repeated, other wise a list of unique
||| names.
...
isModuleNF state (CModuleInst mname xs)
  = InlineModule

isModuleNF state expr
  = state


||| Remove all instances of CModuleInst and merge them together.
mergeModules : (state : List ( String
                             , ( ChannelIR MODULE
                               , List ( String
                                      , ChannelIR CHAN))))
            -> (expr  : ChannelIR type)
...
mergeModules state (CSeq x y)
  = CSeq x (mergeModules state y)

mergeModules state CEnd
    = rebuild CEnd (reduce Nil state)
  where
    ...
    rebuild u ((_,(m,Nil))::xs)
      = (rebuild u xs) -- shouldn't happen
    rebuild u ((_,(m,c::cs))::xs)
      = CSeq (CModuleInst m (c::(fromList cs))) (rebuild u xs)
    ...
    reduce : List (String, (ChannelIR MODULE, List (String, ChannelIR CHAN)))
          -> List (String, (ChannelIR MODULE, List (String, ChannelIR CHAN)))
          -> List (String, (ChannelIR MODULE, List (String, ChannelIR CHAN)))
    reduce new Nil
      = new
    reduce new ((k,(inst, cs))::xs)
      = reduce (action k inst cs new) xs

mergeModules state expr
  = expr

||| Merge all the module instatiations together.
export
