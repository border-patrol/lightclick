...
createModuleBinders state counter (CSeq (CModuleInst mname xs) y) =
  CLet ("lightclick_module_" <+> show counter) (CModuleInst mname xs) (createModuleBinders state (S counter) y)
...
export
covering
freshBinders : (expr  : ChannelIR type) -> Either LightClick.Error (ChannelIR type)
freshBinders expr = runLetFloat (createModuleBinders Nil Z expr)
